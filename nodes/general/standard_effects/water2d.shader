...
// Attributes.
uniform vec4 shadow_color: hint_color = vec4 (1.0); uniform float sine_wave_size = 0.4;
uniform float tile_factor = 10.0; uniform float aspect_ratio = 0.5;
uniform sampler2D texture_offset_uv: hint_black; uniform vec2 texture_offset_scale = vec2 (0.2, 0.2);
uniform float texture_offset_height = 0.1; uniform float texture_offset_time_scale = 0.05;
uniform float sine_time_scale = 0.03; uniform vec2 sine_offset_scale = vec2 (0.4, 0.4);
// Return the sine wave.
vec2 calculate_sine_wave (float time, float multiplier, vec2 uv, vec2 offset_scale) {
	float time_multiplied = (time * multiplier); float unique_offset = (uv.x + uv.y);
	return vec2 (sin (time_multiplied + unique_offset * offset_scale.x),
	cos (time_multiplied + unique_offset * offset_scale.y));
}
// Called on every draw.
void fragment () {
	// Generate water effect.
	vec2 base_uv_offset = (UV * texture_offset_scale); base_uv_offset += (TIME * texture_offset_time_scale);
	vec2 adjusted_uv = (UV * tile_factor); adjusted_uv.y *= aspect_ratio;
