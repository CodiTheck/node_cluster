...
// Specular constants.
const float SPECULAR_SOFT_MIN = 0.0; const float SPECULAR_SOFT_MAX = 0.64;
const float SPECULAR_HARD_MIN = 0.17; const float SPECULAR_HARD_MAX = 0.18;
// Outline constants.
const float OUTLINE_MIN = 0.45; const float OUTLINE_MAX = 0.47;
// Anisotropic constants.
const float ANISOTROPY_SHARPNESS_MIN = 0.34; const float ANISOTROPY_SHARPNESS_MAX = 0.35;
const float ANISOTROPY_SOFTNESS_MIN = 0.06; const float ANISOTROPY_SOFTNESS_MAX = 0.364;
const float ANISOTROPY_HOTSPOT_MIN = 0.083; const float ANISOTROPY_HOTSPOT_MAX = 0.637;
const float ANISOTROPY_BAND_MIN = 0.42; const float ANISOTROPY_BAND_MIDDLE = 0.5;
...
// Light colors.
uniform vec4 base_color: hint_color; uniform sampler2D base_texture: hint_albedo;
// Color masks.
uniform vec4 paint_color1: hint_color = vec4 (1); uniform vec4 paint_color2: hint_color = vec4 (1);
uniform vec4 paint_color3: hint_color = vec4 (1); uniform sampler2D paint_mask1: hint_black;
uniform sampler2D paint_mask2: hint_black; uniform sampler2D paint_mask3: hint_black;
uniform sampler2D shadow_paint: hint_black; uniform vec4 key_light_color: hint_color;
uniform vec4 fill_light_color: hint_color; uniform vec4 kick_light_color: hint_color;
uniform vec4 shadow_color: hint_color;
// Data textures.
uniform sampler2D light_data: hint_black; uniform sampler2D specular_data: hint_black;
uniform sampler2D key_light_ramp: hint_black; uniform sampler2D fill_light_ramp: hint_black;
uniform sampler2D kick_light_ramp: hint_black; uniform sampler2D metalness_texture: hint_black_albedo;
...
// Outline.
uniform float outline_size = 5.0; uniform vec4 outline_color: hint_color = vec4 (0, 0, 0, 1.0);
// Metalness.
...
// Specular.
uniform float specular_size: hint_range (0, 4); uniform vec4 specular_color: hint_color;
...
// Rim light.
uniform float rim_light_softness: hint_range (0, 1) = 0.5; varying vec3 down_camera_angle;
uniform vec4 rim_light_color: hint_color = vec4 (0, 0, 0, 1); uniform float rim_fresnel_power = 3.0;
uniform float rim_normal_offset_x = 0.0; uniform float rim_normal_offset_y = 0.0;
// Generate vertex.
void vertex () {down_camera_angle = (vec4 (0, -1, 0, 1) * CAMERA_MATRIX).xyz;}
// Called on any draw.
void fragment () {
	// Data.
	...
	if (rim_light_color.r > 0.0 || rim_light_color.g > 0.0 || rim_light_color.b > 0.0) {
		float rim_value = pow (1.0 - dot (normalize (NORMAL + vec3 (0,
		rim_normal_offset_y, rim_normal_offset_x)), VIEW), rim_fresnel_power);
		float hard_rim = step (RIM_SHARPNESS, rim_value);
		float soft_rim = smoothstep (RIM_SOFTNESS_MIN, RIM_SOFTNESS_MAX, rim_value);
		vec3 out_rim_light = (vec3 (mix (hard_rim, soft_rim, rim_light_softness))
		* rim_light_color.rgb); out_color += out_rim_light * kick_light_value;
	...
	if (metalness > 0.0) {
		...
		metalness_value = clamp (pow (metalness_value, vec3 (metalness_contrast_factor)),
		0, 1); metalness_value = clamp (metalness_value, dark_metalness_color.rgb,
		light_metalness_color.rgb); out_color = mix (out_color, metalness_value, metalness);
	}
	// Specular.
	float specular = texture (specular_data, SCREEN_UV).r;
	if (specular_size > 0.0) {
		float soft_specular = smoothstep (SPECULAR_SOFT_MIN, SPECULAR_SOFT_MAX,
		(specular * specular_size)); float hard_specular = smoothstep (SPECULAR_HARD_MIN,
		SPECULAR_HARD_MAX, (specular * specular_size)); vec3 specular_out = (mix
		(hard_specular, soft_specular, specular_softness) * specular_color.rgb);
		out_color += specular_out;
	}
	// Anisotropic highlight.
	if (anisotropy_specular_strength > 0.0) {
		float anisotropy_angle = (down_camera_angle.z * 0.33);
		float high_anisotropy_noise_value = ((texture (high_frequency_anisotropy_noise,
		vec2 (UV.x, 0)).r - 0.5) * 0.2);
		float low_anisotropy_noise_value = ((texture (low_frequency_anisotropy_noise,
		vec2 (UV.x, 0)).r - 0.5) * 0.2);
		float anisotropy_specular_hotspot = smoothstep (ANISOTROPY_HOTSPOT_MIN,
		ANISOTROPY_HOTSPOT_MAX, (specular * anisotropy_specular_width));
		float spottiness_anisotropy_noise_value = ((texture (spottiness_anisotropy_noise,
		vec2 (UV.x, 0)).r - 0.5) * anisotropy_specular_contrast + anisotropy_specular_brightness);
		float anisotropy_uv = (UV.y + high_anisotropy_noise_value +
		low_anisotropy_noise_value - anisotropy_angle);
		float lower_sample = smoothstep (ANISOTROPY_BAND_MIN, ANISOTROPY_BAND_MIDDLE,
		anisotropy_uv); float higher_sample = (1.0 - smoothstep (ANISOTROPY_BAND_MIDDLE,
		ANISOTROPY_BAND_MAX, anisotropy_uv));
		...
		* max (anisotropy_specular_hotspot, anisotropy_in_shadow_strength));
		float sharp_anisotropy_value = smoothstep (ANISOTROPY_SHARPNESS_MIN,
		ANISOTROPY_SHARPNESS_MAX, anisotropy_sample);
		float soft_anisotropy_value = smoothstep (ANISOTROPY_SOFTNESS_MIN,
		ANISOTROPY_SOFTNESS_MAX, anisotropy_sample);
