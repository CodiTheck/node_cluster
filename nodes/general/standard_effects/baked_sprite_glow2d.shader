...
// Generate a tint rgba color.
vec4 tint_rgba (vec4 tex, vec4 color) {
	float tint_amount = dot (tex.rgb, vec3 (0.222, 0.707, 0.071)); vec3 tint = (color.rgb * tint_amount);
	tex.rgb = mix (tex.rgb, tint.rgb, color.a); return tex;
}
// Generate alpha intensity.
vec4 alpha_intensity (vec4 tex, float fade) {
	if (tex.a < falloff_max_alpha) tex.a = mix (0.0, tex.a, fade); return tex;
}
// Blend given colors.
vec4 blend (vec4 origin, vec4 overlay, float blend) {
	vec4 o = origin; o.a = (overlay.a + origin.a * (1.0 - overlay.a));
	o.rgb = (overlay.rgb * overlay.a + origin.rgb * origin.a *
	(1.0 - overlay.a)) / (o.a + 0.0000001); o.a = clamp (o.a, 0.0, 1.0);
	o = mix (origin, o, blend); return o;
}
// Called on every draw.
void fragment () {
	// Generate baked glow effect.
	...
	o.rgb *= main_texture.rgb; o.a = o.a * main_texture.a; COLOR = o;
}