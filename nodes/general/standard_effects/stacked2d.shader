// Edited by Obrymec.
shader_type canvas_item; render_mode unshaded; varying lowp float rot;
// Attributes.
uniform sampler2D slice_sheet: hint_black; uniform lowp int columns: hint_range (1, 64);
uniform lowp int rows: hint_range (1, 64); uniform lowp int layer_count: hint_range (1, 64);
uniform lowp float stretch: hint_range (1.0, 3.0); uniform lowp int center: hint_range (0, 63);
const lowp float PI = 3.14159265358979323846; const lowp float HALF_PI = (3.14159265358979323846 / 2f);
// Gets color on layer.
lowp vec4 getColorOnLayer (lowp int layer, lowp vec2 uv) {
	...
	lowp vec4 tex_col = texture (slice_sheet, (layer_start + slice_size * clamp (uv, 0, 1f))); return tex_col;
}
// Generate vertex.
void vertex () {
	...
	layer_space += (floor (float (layer_count - center) * stretch) * up_vector / 2f); VERTEX = layer_space;
}
// Called on every draw.
void fragment () {
	// Generate stacked effect.
	lowp vec2 size_pix = vec2 (textureSize (slice_sheet, 0));
	lowp vec2 LAYER_SIZE_IN_PIXELS = (size_pix / vec2 (ivec2 (columns, rows)));
	lowp vec2 LAYER_PIXEL_SIZE_IN_UV = (1f / LAYER_SIZE_IN_PIXELS);
	lowp vec2 up_vector = vec2 (cos (rot), sin (rot));
	...
	lowp vec2 OldMin = vec2 (0); lowp vec2 OldMax = vec2 (1f); lowp vec2 OldRange = (OldMin - OldMax);
	...
	lowp vec2 NewRange = (NewMin - NewMax); lowp vec4 current_color = vec4 (0);
	...
	for (lowp int i = 0; i < int (float (layer_count) * stretch); i++) {
		...
		current_color = mix (getColorOnLayer (int (float (i) / stretch),
		current_uv), current_color, current_color.a);
	} COLOR = current_color;
}