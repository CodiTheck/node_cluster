// OmniScale by Lior Halphon.
shader_type canvas_item; uniform int scale_multiplier: hint_range (2, 12) = 4;
...
// Check whether two vector are deferent.
...
vec4 scale (sampler2D image, vec2 coord, vec2 pxSize) {
	vec2 OutputSize = vec2 (float (scale_multiplier), float (scale_multiplier));
	vec2 textureDimensions = vec2 (1.0,1.0); vec2 o = pxSize; vec2 texCoord = coord;
	...
	if (p.x > 0.5) {o.x = -o.x; p.x = (1.0 - p.x);} if (p.y > 0.5) {o.y = -o.y; p.y = (1.0 - p.y);}
	...
	int pattern = 0; if (is_different (w0, w4)) pattern |= 1 << 0;
	if (is_different (w1, w4)) pattern |= 1 << 1; if (is_different (w2, w4)) pattern |= 1 << 2;
	if (is_different (w3, w4)) pattern |= 1 << 3; if (is_different (w5, w4)) pattern |= 1 << 4;
	if (is_different (w6, w4)) pattern |= 1 << 5; if (is_different (w7, w4)) pattern |= 1 << 6;
	...
	if ((((pattern & (11)) == (11)) || ((pattern & (254)) == (74)) || ((pattern & (254)) == (26)))
	&& is_different (w3, w1)) return w4;
	if ((((pattern & (111)) == (42)) || ((pattern & (91)) == (10)) || ((pattern & (191)) == (58))
	|| ((pattern & (223)) == (90)) || ((pattern & (159)) == (138)) || ((pattern & (207)) == (138))
	|| ((pattern & (239)) == (78)) || ((pattern & (63)) == (14)) || ((pattern & (251)) == (90))
	|| ((pattern & (187)) == (138)) || ((pattern & (127)) == (90)) || ((pattern & (175)) == (138))
	...
	if (((pattern & (47)) == (47))) {
		...
		if (dist < (0.5 - pixel_size / 2.0)) return w4; vec4 r;
		if (is_different (w0, w1) || is_different (w0, w3)) r = mix (w1, w3, (p.y - p.x + 0.5));
		...
		if (dist > pixel_size / 2.0) return w1; vec4 r = mix (w3, w4, (p.x + 0.5));
		if (dist < -pixel_size / 2.0) return r; return mix (r, w1, (dist + pixel_size / 2.0) / pixel_size);
	} if (((pattern & (219)) == (73)) || ((pattern & (239)) == (109))) {
		float dist = (p.y - 2.0 * p.x);
		float pixel_size = (length (1.0 / (OutputSize / textureDimensions)) * sqrt (5.0));
		if ((p.y - 2.0 * p.x) > pixel_size / 2.0) return w3; vec4 r = mix (w1, w4, (p.x + 0.5));
		if (dist < (-pixel_size / 2.0)) return r; return mix (r, w3, (dist + pixel_size / 2.0) / pixel_size);
	} if (((pattern & (191)) == (143)) || ((pattern & (126)) == (14))) {
		float dist = (p.x + 2.0 * p.y);
		float pixel_size = (length (1.0 / (OutputSize / textureDimensions)) * sqrt (5.0));
		if (dist > (1.0 + pixel_size / 2.0)) return w4; vec4 r;
		if (is_different (w0, w1) || is_different (w0, w3)) r = mix (w1, w3, (p.y - p.x + 0.5));
		else r = mix (mix ((w1 * 0.375 + w0 * 0.25 + w3 * 0.375), w3, (p.y * 2.0)), w1, (p.x * 2.0));
		if (dist < (1.0 - pixel_size / 2.0)) return r;
		return mix (r, w4, (dist + pixel_size / 2.0 - 1.0) / pixel_size);
	} if (((pattern & (126)) == (42)) || ((pattern & (239)) == (171))) {
		float dist = (p.y + 2.0 * p.x);
		float pixel_size = (length (1.0 / (OutputSize / textureDimensions)) * sqrt (5.0));
		if ((p.y + 2.0 * p.x) > (1.0 + pixel_size / 2.0)) return w4; vec4 r;
		if (is_different (w0, w1) || is_different (w0, w3)) r = mix (w1, w3, (p.y - p.x + 0.5));
		else r = mix (mix ((w1 * 0.375 + w0 * 0.25 + w3 * 0.375), w3, (p.y * 2.0)), w1, (p.x * 2.0));
		if (dist < (1.0 - pixel_size / 2.0)) return r;
		return mix (r, w4, (dist + pixel_size / 2.0 - 1.0) / pixel_size);
	} if (((pattern & (27)) == (3)) || ((pattern & (79)) == (67)) || ((pattern & (139)) == (131))
	|| ((pattern & (107)) == (67))) return mix (w4, w3, (0.5 - p.x));
	if (((pattern & (75)) == (9)) || ((pattern & (139)) == (137)) || ((pattern & (31)) == (25))
	|| ((pattern & (59)) == (25))) return mix (w4, w1, (0.5 - p.y));
	if (((pattern & (251)) == (106)) || ((pattern & (111)) == (110)) || ((pattern & (63)) == (62))
	|| ((pattern & (251)) == (250)) || ((pattern & (223)) == (222)) || ((pattern & (223)) == (30)))
		return mix (w4, w0, (1.0 - p.x - p.y) / 2.0);
	if (((pattern & (79)) == (75)) || ((pattern & (159)) == (27)) || ((pattern & (47)) == (11))
	|| ((pattern & (190)) == (10)) || ((pattern & (238)) == (10)) || ((pattern & (126)) == (10))
	|| ((pattern & (235)) == (75)) || ((pattern & (59)) == (27))) {
		float dist = (p.x + p.y); float pixel_size = length (1.0 / (OutputSize / textureDimensions));
		if (dist > (0.5 + pixel_size / 2.0)) return w4; vec4 r;
		if (is_different (w0, w1) || is_different (w0, w3)) r = mix (w1, w3, (p.y - p.x + 0.5));
		else r = mix (mix ((w1 * 0.375 + w0 * 0.25 + w3 * 0.375), w3, (p.y * 2.0)), w1, (p.x * 2.0));
		if (dist < (0.5 - pixel_size / 2.0)) return r;
	...
	if (is_different (x0, w4)) pattern |= 1 << 8; if (is_different (x1, w4)) pattern |= 1 << 9;
	if (is_different (x2, w4)) pattern |= 1 << 10; if (is_different (x3, w4)) pattern |= 1 << 11;
	if (is_different (x4, w4)) pattern |= 1 << 12; if (is_different (x5, w4)) pattern |= 1 << 13;
	if (is_different (x6, w4)) pattern |= 1 << 14; int diagonal_bias = -7;
	while (pattern != 0) {diagonal_bias += (pattern & 1); pattern >>= 1;}
	if (diagonal_bias <= 0) {
		vec4 r = mix (w1, w3, (p.y - p.x + 0.5)); if (dist < (0.5 - pixel_size / 2.0)) return r;
		return mix (r, w4, (dist + pixel_size / 2.0 - 0.5) / pixel_size);
	} return w4;
}
// Called on any draw.
void fragment () {COLOR = scale (TEXTURE, UV, TEXTURE_PIXEL_SIZE);}