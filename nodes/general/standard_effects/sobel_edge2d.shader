...
// Get SobelEdge generated value.
float sobelEngeFunc (sampler2D _tex_sobel, vec2 _uv_sobel, float _lod_sobel,
float _ofst_sobel, bool _preconv_grayscale_sobel, bool _use_scharr) {
	vec3 s00 = vec3 (0.0); vec3 s01 = vec3 (0.0); vec3 s02 = vec3 (0.0);
	vec3 s10 = vec3 (0.0); vec3 s12 = vec3 (0.0); vec3 s20 = vec3 (0.0);
	vec3 s21 = vec3 (0.0); vec3 s22 = vec3 (0.0);
	if (_lod_sobel < 0.0) {
	...
	if (!_use_scharr) {
		...
		float scharrX = (3.0 * s00.x + 10.0 * s10.x + 3.0 * s20.x - 3.0 * s02.x
		- 10.0 * s12.x - 3.0 * s22.x); float scharrY = (3.0 * s00.x + 10.0 * s01.x
		+ 3.0 * s02.x - 3.0 * s20.x - 10.0 * s21.x - 3.0 * s22.x);
		edgeSqr = (scharrX * scharrX + scharrY * scharrY);
	} return edgeSqr;
}
// Called on every draw.
void fragment () {
	// Generate sobel edge effect.
	vec3 n_out4p0 = vec3 (UV, 0.0); float n_out5p0 = -1.000000;
	float n_out6p0 = 0.005000; bool n_out7p0 = false;
	bool n_out8p0 = false; float n_out2p0; {
