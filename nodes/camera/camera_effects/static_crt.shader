// Created by hiulit.
shader_type canvas_item; const float PI = 3.14159265359;
// Attributes.
...
vec2 uv_curve (vec2 uv) {
	if (show_curvature) {
		...
		uv = (uv + uv * offset * offset); uv = (uv * 0.5 + 0.5);
	} return uv;
}
// This method is called on any draw.
void fragment () {
	// Generate a Static CRT effect.
	vec2 uv = uv_curve (UV); vec2 screen_uv = uv_curve (SCREEN_UV);
	vec3 color = texture (SCREEN_TEXTURE, screen_uv).rgb;
	// For aberation effect.
	...
	// Horizontal or vertical scanlines.
	if (show_horizontal_scan_lines || show_vertical_scan_lines) color *= boost;
	// Fill the blank space of the corners, left by the curvature, with black.
	if (uv.x < 0.0 || uv.x > 1.0 || uv.y < 0.0 || uv.y > 1.0) color = corner_color.rgb;
