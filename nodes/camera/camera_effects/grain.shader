// SHADER ORIGINALY CREADED BY "spl!te".
shader_type canvas_item; uniform vec2 screen_size; uniform bool colored = true; varying float time;
uniform float color_amount: hint_range (0, 1.3) = 1.3;
uniform float grain_amount: hint_range (0, 0.07) = 0.025;
uniform float grain_size: hint_range (1, 3) = 1.6; uniform float lum_amount: hint_range (0, 2) = 2;
// Compute the time.
void vertex () {time = TIME;}
// Random texture generator.
vec4 rnm (vec2 tc) {
	float noise = (sin (dot ((tc + vec2 (time, time)), vec2 (12.9898, 78.233))) * 43758.5453);
	float noiseR = ((fract (noise) * 2.0) - 1.0); float noiseG = ((fract (noise * 1.2154) * 2.0) - 1.0); 
	float noiseB = ((fract (noise * 1.3453) * 2.0) - 1.0); 
	float noiseA = ((fract (noise * 1.3647) * 2.0) - 1.0); return vec4 (noiseR, noiseG, noiseB, noiseA);
}
// Make a fade effect.
float fade (float t) {return (pow (t, 3) * ((t * ((t * 6.0) - 15.0)) + 10.0));}
// Calculate the pnoise 3d.
float pnoise3D (vec3 p) {
	vec3 pi = (0.00390625 * floor (p));
	pi = vec3 ((pi.x + 0.001953125), (pi.y + 0.001953125), (pi.z + 0.001953125));
	vec3 pf = fract (p); float perm00 = rnm (pi.xy).a;
	// Noise contributions.
	...
	float n_xyz = mix (n_xy.x, n_xy.y, fade (pf.z)); return n_xyz;
}
...
vec2 coordRot (vec2 tc, float angle) {
	...
	rotX = ((rotX / aspect) * 0.5 + 0.5); rotY = (rotY * 0.5 + 0.5); return vec2 (rotX, rotY);
}
// This method is called on any draw.
void fragment () {
	// Generate grain effect.
	vec3 rotOffset = vec3 (1.425, 3.892, 5.835); vec2 rotCoordsR = coordRot (UV, (time + rotOffset.x));
	vec3 noise = vec3 (pnoise3D (vec3 (rotCoordsR * vec2 ((screen_size.x / grain_size),
	(screen_size.y / grain_size)), 0.0)));
	if (colored) {
		vec2 rotCoordsG = coordRot (UV, (time + rotOffset.y));
		vec2 rotCoordsB = coordRot (UV, (time + rotOffset.z));
		noise.g = mix (noise.r, pnoise3D (vec3 (rotCoordsG * vec2 ((screen_size.x / grain_size),
		(screen_size.y / grain_size)), 1.0)), color_amount);
		noise.b = mix (noise.r, pnoise3D (vec3 (rotCoordsB * vec2 ((screen_size.x / grain_size),
		(screen_size.y / grain_size)), 2.0)), color_amount);
	} vec3 col = texture (TEXTURE, UV).rgb; vec3 lumcoeff = vec3 (0.299, 0.587, 0.114);
	float luminance = mix (0.0, dot (col, lumcoeff), lum_amount);
	float lum = smoothstep (0.2, 0.0, luminance); lum += luminance;
	noise = mix (noise, vec3 (0.0), pow (lum, 4.0)); col = ((col + noise) * grain_amount);
