...
shader_type canvas_item; const float PI = 3.1415926535;
// Attributes.
...
uniform float white_noise_rate: hint_range (0.0, 1.0) = 0.2;
// Make a randomize.
...
void fragment () {
	...
	float enable_color = float (0.0 <= texture_fixed_uv.x &&
	texture_fixed_uv.x <= 1.0 && 0.0 <= texture_fixed_uv.y && texture_fixed_uv.y <= 1.0);
	COLOR.rgb = ((
	...
	COLOR.rgb = mix (
		COLOR.rgb, vec3 (white_noise, white_noise, white_noise), (white_noise_rate * enable_color));
	...
	COLOR = clamp (
		COLOR * vec4 (
			...
			1.0), vec4 (0.0, 0.0, 0.0, 0.0), vec4 (1.0, 1.0, 1.0, 1.0));
	COLOR = mix (
